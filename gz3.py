# -*- coding: utf-8 -*-
import requests,os,shutil,sys,threading

proxy = {'http':'http://127.0.0.1:8002'}

#代理下載，先試 png ，再試 jpg ，服務器不存在文件時返回 True 以便跳出循環。
def pDown(cLink, cPath):
	r = requests.get(cLink+".png", proxies=proxy, stream=True, timeout=None)
	if r.status_code == 200:
		with open(cPath, 'wb') as fd:
			for chunk in r.iter_content(chunk_size=1024):
				fd.write(chunk)
	elif r.status_code == 404:
		r = requests.get(cLink+".jpg", proxies=proxy, stream=True, timeout=None)
		if r.status_code == 200:
			with open(cPath, 'wb') as fd:
				for chunk in r.iter_content(chunk_size=1024):
					fd.write(chunk)
